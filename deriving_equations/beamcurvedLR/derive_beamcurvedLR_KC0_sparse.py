...
var('xi', real=True)
sympy.var('hy, hz, dy, dz, L, E, Iyy, Izz, Iyz, G, A, Ay, Az, J, Rxy', real=True, positive=True)
...
#epsilon = u,x + 1/R*v # NOTE modified according to kinematics of curved structures
#kappay = -theta,x = -rz,x
#kappaz = psi,x = ry,x
#gammay = v,x - theta - 1/R*u = v,x - rz - 1/R*u # NOTE modified according to kinematics of curved structures
#gammaz = w,x + psi = w,x + ry
#kappax = phi,x
# putting in a BL matrix
BL = Matrix([
    2/L*Nu.diff(xi) + 1/Rxy*Nv,
    -2/L*Nrz.diff(xi),
    2/L*Nry.diff(xi),
    2/L*Nv.diff(xi) - Nrz - 1/Rxy*Nu,
    2/L*Nw.diff(xi) + Nry,
    2/L*Nrx.diff(xi)])
